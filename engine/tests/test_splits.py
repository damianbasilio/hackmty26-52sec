...
class World:
    def __init__(self, monkeypatch):
        ...
        m.setattr(repository, "fetch_current_customer", lambda: CREATOR)
...
def _join(world, participant_id, name, **overrides):
    ...
    body.update(overrides)
    return client.post("/splits/join", json=body)


def _pay(participant_id, transfer_id="trf_split_0001", account_id=BETO_ACCOUNT["id"]):
    return client.post(
...
def test_retrying_a_payment_replays_the_same_transfer(world):
    ...
    assert _pay("spp_beto_0001", transfer_id="trf_split_0002").status_code == 409
...
def test_the_creator_does_not_pay_themselves(world):
    ...
    assert _pay("spp_spl_test_0001_creator").status_code == 422
    assert world.transfer_calls == []


def test_a_known_participant_pays_only_from_their_own_account(world):
    _create()
    _join(world, "spp_luis_0001", "Luis", customer_id="cus_nessie_luis")

    assert _pay("spp_luis_0001").status_code == 422
    assert world.transfer_calls == []
